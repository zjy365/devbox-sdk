...
if run_test "GET" "/api/v1/files/read?path=test_tmp/nonexistent.txt" "" "200" "Read File (nonexistent)" "false"; then ((PASSED_TESTS++)); fi
((TOTAL_TESTS++))
...
if run_test "GET" "/api/v1/files/read?path=test_file.txt" "" "200" "Read File (successful)" "true"; then ((PASSED_TESTS++)); fi
((TOTAL_TESTS++))
